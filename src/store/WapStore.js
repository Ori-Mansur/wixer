...
export default {
  ...
  mutations: {
    ...
    updateTxt(state, { data }) {
      ...
      const dataIdx = state.currWap.sections[idx].data.findIndex(
        currData => currData._id === data.txt._id
      );
    ...
    addWidget(state, { data }) {
      const idx = state.currWap.sections.findIndex(
        currSection => currSection._id === data.sectionId
      );
    ...
    setBcgImg(state, { sectionData }) {
      const idx = state.currWap.sections.findIndex(
        section => section._id === sectionData.id
      );
    ...
    updateStyle(state, { data }) {
      ...
      if (data.cardData.id) {
        const cardIdx = state.currWap.sections[idx].data.findIndex(
          card => card._id === data.cardData.id
        );
        state.currWap.sections[idx].data[cardIdx];
        state.currWap.sections[idx].data[cardIdx].data[
          data.cardData.idx
        ].style = data.cardData.style;
      } else {
        state.currWap.sections[idx].data[data.cardData.idx].style =
          data.cardData.style;
      }
    }
  },
  actions: {
    ...
    async setBcgImg(context, { data }) {
      ...
      context.commit({
        type: 'setBcgImg',
        sectionData: { id: data.sectionId, imgUrl }
      });
    ...
    async setCardImg(context, { data }) {
      ...
      context.commit({
        type: 'setCardImg',
        sectionData: { id: data.sectionId, idx: data.idx, imgUrl }
      });
      context.commit({ type: 'success', msg: 'Image upload' });
    }
  },
